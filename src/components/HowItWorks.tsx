const WaIcon = () => (
  <svg className="w-9 h-9" fill="#25D366" viewBox="0 0 24 24">
...
const steps = [
  ...
  },
  {
    ...
    title: "Choose Your Daily Plan",
    desc: "Simple monthly subscription at just ₹449",
    iconBg: "bg-tan p-0 overflow-hidden",
  },
  {
    num: "3",
    icon: (
    ...
    title: "Wake Up To Fresh Flowers",
    desc: "Delivered to your doorstep before 7 AM",
    iconBg: "bg-tan p-0 overflow-hidden",
  },
];
          ...
          {steps.map((step, i) => (
            <div key={step.num} className={`flex flex-col items-center gap-4 group reveal reveal-delay-${i + 1}`}>
              ...
              <div className="relative">
                {/* Step number badge */}
                <div className="absolute -top-2 -left-2 z-10 w-7 h-7 rounded-full bg-[#C69C6D] text-white text-[12px] font-bold flex items-center justify-center shadow-md">
                  {step.num}
                </div>
                <div className={`w-[112px] h-[112px] rounded-3xl shadow-lg flex items-center justify-center border border-gold-light/30 ${step.iconBg} p-5 group-hover:-translate-y-2 transition-transform duration-300`}>
                  {step.icon}
                </div>
              </div>
              <div>
                <h3 className="font-serif text-[1.05rem] font-semibold text-primary mb-1.5">{step.title}</h3>
                <p className="text-[13px] text-muted leading-relaxed max-w-[180px] mx-auto">{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
